...
average_DLUser = pd.read_csv('sample-folder/IOH - Average of DLUser - MC.csv')

list1 = average_DLUser.columns[0:]
result = pd.DataFrame()

for x in range(2, len(average_DLUser.columns), 1):

    sitelist = average_DLUser[['Sector', 'Category']]
    duration_week = pd.DataFrame(data={'date': [list1[x]] * len(average_DLUser.index)})
    content = pd.DataFrame(average_DLUser.iloc[:,x]).reset_index(drop=True)
    content.rename({content.columns[0]:'DLUser'}, axis=1, inplace=True)
...
result.rename(columns={'DLUser': 'y', 'StartOfWeek':'ds'}, inplace=True)
...
concatenate.to_excel('result/prophet-MC-DLUser-result.xlsx')
