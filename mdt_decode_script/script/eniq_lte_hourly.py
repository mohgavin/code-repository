...
def get_data(dict_id):
    ...
    try:
        ...
        #print('   Query success........')
        df.to_csv("%s/hourly_cell_%s_%02d.csv" %(path_daily, s_date2, s_hour), index=None)
    ...
    for dict_hour in list_hour:
        ...
        for dict_id in list_data:
            if (dict_id['date2'] == s_date2) and (dict_id['hour'] == hour):
                s_port = dict_id['eniq']['enid']
                path_daily = '/var/opt/common5/eniq/%s/%s'%(s_port, s_date2)
                file = "%s/hourly_cell_%s_%02d.csv" %(path_daily, s_date2, hour)
        ...
        if len(list_df) > 0:
            ...
            path_daily = '/var/opt/common5/eniq/hourly/%s'%s_date2
...
if __name__ == '__main__':
    ...
    for dt in rrule.rrule(rrule.HOURLY, dtstart=start_datetime, until=stop_datetime):
        ...
        folder_hour = '/var/opt/common5/eniq/hourly/%s'%(s_date2)
