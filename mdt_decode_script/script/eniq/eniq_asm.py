...
def combineData(list_data):
    ...
    if len(list_df)> 0:
        ...
        if os.path.exists(file_power):
            ...
            print('   Found eutrancellfdd: %s and %s still not have cellname'%(len(df_asm.loc[pd.notnull(df_asm['eutrancellfdd'])]), len(df_asm.loc[pd.isnull(df_asm['eutrancellfdd'])])))

            df_power['sectorCarrierId'] = pd.to_numeric(df_power["sectorCarrierId"], errors='coerce').fillna(0)
            df_power['id'] = df_power['NodeId'].astype('str') + '_' + df_power['sectorCarrierId'].astype('int').astype('str')
            dict_cellname = df_power.set_index('id').to_dict()['EUtranCellFDDId']
            df_asm['sectorcarrier'] = pd.to_numeric(df_asm["sectorcarrier"], errors='coerce').fillna(0)
            df_asm['id'] = df_asm['erbs'].astype('str') + '_' + df_asm['sectorcarrier'].astype('int').astype('str')
            df_asm['eutrancellfdd2'] =  df_asm['id'].map(dict_cellname)
            df_asm['eutrancellfdd'] = df_asm['eutrancellfdd'].fillna(df_asm['eutrancellfdd2'])
            print('   Found eutrancellfdd: %s and %s still not have cellname'%(len(df_asm.loc[pd.notnull(df_asm['eutrancellfdd'])]), len(df_asm.loc[pd.isnull(df_asm['eutrancellfdd'])])))
