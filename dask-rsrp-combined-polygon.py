...
if __name__ == '__main__':
	...
	grid = dask_pd.read_csv('grid_folder/mangkuluhur.csv')
	...
	dask_gdf_mdt['combined'] = dask_gdf_mdt['site'].astype(str) + "@" + dask_gdf_mdt['enodebid'].astype(str) + "@" + dask_gdf_mdt['ci'].astype(str) + "@" + dask_gdf_mdt['WKT_polygon'].astype(str) + "@" + dask_gdf_mdt['altitude'].astype(str) + "@" + dask_gdf_mdt['date'].astype(str)
	...
	#pivot = pivot.compute()
	pivot_mean.to_csv('result/rsrp-combined-polygon.csv')
	pivot_count.to_csv('result/rsrp-combined-polygon-pop.csv')
